...
namespace Zzh.Lib.DB.Repositorys
{
    public static class Sys_MenuRepository 
    {
        ...
        {
            var list = await (from j in visiter.DB.Sys_Menus
        ...
        {
            var list = (from j in visiter.DB.Sys_Menus
                        orderby j.MenuSortID, j.MenuId
                        select j).ToList();
            return list;
        }
        ...
        {
            var list = (from j in visiter.DB.Sys_Menus
                        join roleMenu in visiter.DB.Sys_RoleMenus on j.MenuId equals roleMenu.MenuId
        ...
        {
            var list = await visiter.DB.Sys_Menus.OrderBy(p=>p.MenuSortID).ToListAsync();
        ...
        public static async Task<Tuple<int, List<Sys_Menu>>> GetListAsync(RepositoryVisiter visiter, int page, int rows, string menuName, int parentId)
        {
            ...
            var total = await (from j in visiter.DB.Sys_Menus
        ...
        {
            return await visiter.DB.Sys_Menus.Where(p => p.MenuId == menuId).FirstOrDefaultAsync();
        }
        public static async Task<bool> AddOrUpdateAsync(RepositoryVisiter visiter, Sys_Menu menu)
        {
            try
            {
                ...
                if (isNew)
                    visiter.DB.Sys_Menus.Add(sysMenu);
        ...
        {
            var menu = await visiter.DB.Sys_Menus.Where(p => p.MenuId == menuId).FirstOrDefaultAsync();
            if (menu != null)
            {
                visiter.DB.Sys_Menus.Remove(menu);
                return await visiter.DB.SaveChangesAsync() == 1;
            }
            return false;
        }
        ...
        {
            return await visiter.DB.Sys_Menus.Where(p => p.ParentId == parentId).OrderBy(p => p.MenuSortID).ToListAsync();
        }
        public static async Task<List<Sys_Menu>> GetAllChildMeunList(RepositoryVisiter visiter)
        {
            return await visiter.DB.Sys_Menus.Where(p => p.ParentId > 0).ToListAsync();
        }
    }
}
