...
{
    public class BaseORCLRepository : IDisposable
    {
        public readonly ProContext_ORCL context;

        static object _syncObject = new object();

        public BaseORCLRepository()
        {
            context = new ProContext_ORCL();
        }
        public BaseORCLRepository(string conn)
        {
            context = new ProContext_ORCL(conn);
        }
